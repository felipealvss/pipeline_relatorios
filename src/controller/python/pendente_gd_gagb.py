...
def atualizar_relatorio(log_file):
    try:
        atualiza_relatorio(
            assunto=config.LOG_CONTEXT_CONFIG['pendentes_gd_gagb'],
            origin_excel_file=config.DIRECTORIES['dir_origem_rel'],
            name_excel_file=config.FILE_CONFIG['arq_rel_pendentes_gd_gagb'],
            log_file=log_file
        )
...
def extrair_imagem_relatorio(log_file):
    try:
        extrai_imagem_relatorio(
            ...
            nome_arquivo_excel=config.FILE_CONFIG['arq_rel_pendentes_gd_gagb'],
            caminho_arquivo_imagem=config.DIRECTORIES['dir_final_img'],
            nome_arquivo_imagem=config.FILE_CONFIG['arq_img_pendentes_gd_gagb'],
            aba=config.WORKSHEET_TAB['tab_pendentes_gd_gagb'],
            intervalo=config.WORKSHEET_RANGE['range_pendentes_gd_gagb'],
            log_file=log_file
        )
    except Exception as e:
