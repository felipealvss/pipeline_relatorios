...
def knime_atualiza_pend_gb_rede_roubada(assunto, log_file):
    ...
    arquivo_destino = os.path.join(
        ...
        config.FILE_CONFIG["arq_rel_pendentes_gb_rede_roub"]
    )
    ...
    try:
        ...
        if aba_destino is None:
            ...
            aba_destino.Name = 'REDE_ROUBADA'

        # Acessar as abas desejadas
        aba_origem = wb_origem.Sheets('REDE_ROUBADA')

        # 1. Limpar os dados da aba "knime_pend_gb"
        aba_destino.Cells.ClearContents()  # Limpar apenas o conteúdo (sem afetar formatação, gráficos, etc.)
        ...
        # Agora, tentamos copiar os dados para a aba de destino em bloco
        if dados_origem:
            aba_destino.Range(aba_destino.Cells(1, 1), aba_destino.Cells(ultima_linha, ultima_coluna)).Value = dados_origem
            log_event(f"{assunto} Dados copiados para '{aba_destino.Name}'.")
            ...
            tabela = aba_destino.ListObjects.Add(1, aba_destino.Range(aba_destino.Cells(1, 1), aba_destino.Cells(ultima_linha, ultima_coluna)))

            tabela.Name = "REDE_ROUBADA"
        ...
        try:
            ...
            #     log_event(f"{assunto} Validação bem-sucedida: {linhas_destino} linhas e {colunas_destino} colunas copiados para 'knime_pend_gb_red_roub'.")
            # else:
